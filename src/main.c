...
static void scan_one(uint32_t f, uint8_t sql) {
    USB_CDC_Poll();
    if (black_has(f)) return;
    BK4819_TuneTo(f, 0); BK4819_Squelch(sql, f, 1, 1); SYSTICK_DelayUs(10000);
    uint16_t r = BK4819_GetRSSI(); uint8_t n = BK4819_GetNoise();
    if (n > 46) return;
}
static void scan_entry(const ScanEntry *e) {
    uint32_t step = e->freq_end ? (e->step_khz ? e->step_khz * 100 : 2500) : 0;
    if (step) { for (uint32_t f = e->freq; f <= e->freq_end; f += step) scan_one(f, e->squelch); }
    else { scan_one(e->freq, e->squelch); }
}
...
static void cmd_CH_LS(const char *a) { (void)a;
    ...
    for (uint16_t i = 0; i < gChannelCount; i++) {
        ...
        if (e->freq_end) OUTF("  %u: %lu.%05lu-%lu.%05lu", i, e->freq / 100000, e->freq % 100000, e->freq_end / 100000, e->freq_end % 100000);
        else OUTF("  %u: %lu.%05lu", i, e->freq / 100000, e->freq % 100000);
    }
}
...
static void cmd_SL_LS(const char *a) { (void)a; OUTF("active SL %u items", gScanCount); for (uint16_t i=0; i<gScanCount; i++) { uint16_t ci=gScanList[i]; if (ci<gChannelCount) OUTF("  %u->%lu.%05lu",i,gChannels[ci].freq/100000,gChannels[ci].freq%100000); } }
static void cmd_G(const char *a) { (void)a; if (gScanCount==0) { OUT("NO SL\n"); return; } for (uint16_t i=0;i<gScanCount;i++) { uint16_t ci=gScanList[i]; if (ci<gChannelCount) scan_entry(&gChannels[ci]); } OUT("DONE\n"); }
static void cmd_SCAN_SL(const char *a) { while (*a==' ') a++; char name[32]="scanlist_"; int ni=9; while (*a && *a!=' ' && ni<30) name[ni++]=*a++; name[ni]=0; ScanListIdx sl; sl.count=0; Storage_Load(name,0,&sl,sizeof(sl)); if (sl.count==0) { OUTF("NO %s",name); return; } for (uint16_t si=0;si<sl.count;si++) { uint16_t ci=sl.idx[si]; if (ci<gChannelCount) scan_entry(&gChannels[ci]); } OUT("DONE\n"); }
static void cmd_LOOP(const char *a) { while (*a==' ') a++; char name[32]="scanlist_"; int ni=9; while (*a && *a!=' ' && ni<30) name[ni++]=*a++; name[ni]=0; ScanListIdx sl; sl.count=0; Storage_Load(name,0,&sl,sizeof(sl)); if (sl.count==0) { OUTF("NO %s",name); return; } while (1) { for (uint16_t si=0;si<sl.count;si++) { uint16_t ci=sl.idx[si]; if (ci<gChannelCount) scan_entry(&gChannels[ci]); } } }
...
static void cmd_BL_RD(const char *a) { (void)a;
    uint16_t cnt=0; Storage_Load("blacklist.sl",0,&cnt,2);
    if (cnt>0 && cnt<=BL_MAX) { Storage_Load("blacklist.sl",1,gBlack,cnt*sizeof(BlackItem)); gBlackCount=(uint8_t)cnt; }
    else gBlackCount=0;
    OUTF("%u",gBlackCount);
}
...
static void cmd_PR_LS(const char *a) { (void)a;
    for (uint8_t i=0;i<MAX_RX_PROFILES;i++) {
        char b[32];
        int n = snprintf_(b,32,"%u %u\n",(unsigned)i,(unsigned)gRxProfiles[i].freq_anchor);
        USB_CDC_Write((uint8_t*)b,(n>0)?n:0);
    }
}
static void cmd_K(const char *a) { uint32_t reg=0; sscanf(a,"%lx",&reg); OUTF("R%02lX=0x%04X",reg,BK4819_ReadRegister((BK4819_REGISTER_t)reg)); }
static void cmd_K_WR(const char *a) { uint32_t reg=0,val=0; sscanf(a,"%lx %lx",&reg,&val); BK4819_WriteRegister((BK4819_REGISTER_t)reg,(uint16_t)val); OUTF("R%02lX=0x%04lX",reg,val); }
...
int main(void) {
    ...
    char line[80]; uint8_t lpos = 0; uint32_t tScr = 0;
